...
class Candidat:
    ...
    def patch_signal(self,y):
        ...
        lh= y[l-10:l+10].mean()
        rh =y[r-10:r+10].mean()
        A = (rh-lh)/(self.x[r]-self.x[l])
        #y[r:l] = A*(self.x[r:l]-self.x[l])+y[l]
        #A = y[l:r].mean()
        for i in range(l,r):
           y[i] = A*(self.x[i]-self.x[l])+lh
...
hight
    ...
    def patch(self,x,y):
        pathced_y= y
        candidat_list = self.get_candidat(x,y,squared=True)
    ...
    def hight_accurance_candidat(self,c,x0,y0):
        ...
        dy = -np.diff(yg)
        newi = find_peaks(dy)[0][0]
        return Candidat(newi+l_idx,x0,y0)
