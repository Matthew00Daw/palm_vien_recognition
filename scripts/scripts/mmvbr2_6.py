...
from scipy.signal import cwt, morlet2, find_peaks,decimate
import sys
sys.path.append('../')
...
class GetPeaks(BaseMethod):
    ...
    def signal_filter(self,x,y):
        ...
        y_d = decimate(y,self.resemple,3)
        #dy = gaussian_filter1d(dy,self.kernel_size/resamle)
        #ks =self.kernel_size*2
        return x[::self.resemple],y_d
   ...
   #     peack += [0 for x in range(self.__n_peaks-len(peack))]
    ...
    def get_candidat(self,x,y):
        dy = -diff(x,y)
        dy = gaussian_filter1d(dy,self.kernel_size/self.resemple)
