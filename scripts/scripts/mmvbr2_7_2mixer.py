...
from .filters import mixer_shmooth
...
class GetPeaks(BaseMethod):
    ...
    def signal_filter(self,x,y):
        ...
        #_,dy = mixer_shmooth(x,y,self.kernel_size)
        dy = savgol_filter(y, self.kernel_size, 3)
        return x,dy
   ...
   #     peack += [0 for x in range(self.__n_peaks-len(peack))]
    ...
    def diff_smooth(self,x,y):
        dy = -diff(x,y)
        _,dy = mixer_shmooth(x,dy,self.kernel_size2)
        return x, dy # gaussian_filter1d(dy,self.kernel_size)
