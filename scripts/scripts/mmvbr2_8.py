...
class GetPeaks(BaseMethod):
    ...
    def adaptive_shooth(self,x,dy,DISTANCE=200):
        y2 = dy.copy()
        for i in range(100):
            candidat_list = self.get_candidat(x,y2)
            #candidat_list = self.select_uniq(candidat_list)
            for c in candidat_list:
                count = 0
                for n in candidat_list:
                    if abs(n.idx-c.idx)<DISTANCE:
                        count+=1
                c.nebrhood = count
            cl = list(filter(lambda x:x.nebrhood>1,candidat_list))
            if len(cl)==0:
                break
            cl = sorted(cl,key=lambda x:-x.nebrhood)
            for c in cl:
                x_tmp =x[c.idx-DISTANCE:c.idx+DISTANCE]
                y_tmp = y2[c.idx-DISTANCE:c.idx+DISTANCE]
                y2[c.idx-DISTANCE:c.idx+DISTANCE] = diffusion_smooth(x_tmp,y_tmp,N=500)
        print(i,cl)
        return y2
    ...
    def execute(self,xx, signal):
        x,dy0 = self.signal_filter(xx.tolist(),signal.tolist())
        _,dy = self.diff_smooth(x,dy0)
        #y2 = self.adaptive_shooth(x,dy)
        candidat_list = self.get_candidat(x,dy)
        self._candidat_list = candidat_list
        candidat_list = self.select_uniq(candidat_list)
        candidat_list = self.til_fat_filter(candidat_list)
   ...
   #     peack += [0 for x in range(self.__n_peaks-len(peack))]
    ...
    def diff_smooth(self,x,y):
        dy = -diff(x,y)
        return x,  gaussian_filter1d(dy,self.kernel_size2)
    ...
    def plot(self,x,y,fig=None,ax=None):
        if not fig:
            fig,ax = plt.subplots(6,sharex=True,figsize=(14,8))
        ...
        candidata_list= self.get_candidat(x,dy)
        candidata_list= self.select_uniq(candidata_list)

        dy2 = self.adaptive_shooth(x,dy)

        candidata_list = self.get_candidat(x,dy2)
        candidata_list = self.select_uniq(candidata_list)


        ax[4].set_title('5. Адоптивное сглаживание')
        ax[5].set_title('5. Адоптивное сглаживание')
        ax[4].plot(x2,dy2)
        ax[5].plot(x2,dy2)
        ...
        for c  in candidata_list:
            px = c.get_lambda()
            py = dy2[c.idx]
            ax[5].plot([px,px],[py,py],ls='',marker='o',color="k")
            ax[5].text(px,py*1.01,'{:1.2f}'.format(c.get_weight()))
        for c in peack:
            px = x[int(c)]
